...
1.  **Flexible**: Người dùng truy cập HTTPS qua Cloudflare, nhưng Cloudflare gọi về VPS qua HTTP (Cổng 80). Đây là cách **DỄ NHẤT** ban đầu.
2.  **Full / Full (Strict)**: Cloudflare gọi về VPS qua HTTPS (Cổng 443). Bạn cần cài **Cloudflare Origin Certificate** trên VPS để dùng chế độ này.

### Cách cài đặt Cloudflare Origin SSL (Để dùng chế độ Full/Strict):

**Bước 1: Lưu chứng chỉ trên VPS**
Tạo file CRT và KEY (nội dung lấy từ bảng điều khiển Cloudflare):
```bash
sudo nano /etc/ssl/certs/cloudflare_origin.crt
sudo nano /etc/ssl/private/cloudflare_origin.key
sudo chmod 600 /etc/ssl/private/cloudflare_origin.key
```

**Bước 2: Cấu hình Nginx (Mẫu chuẩn)**
```nginx
server {
    listen 80;
    server_name trafficuser.live;
